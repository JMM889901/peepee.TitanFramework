...
global function CreateModdedTitanItem
global function CreateGenericItem
global function AddGenericItem
...
global const string FRAMEWORK_VERSION = "2.1.0"
...
struct
{
	...
	//array<array<GlobalItemRef> > globalItemRefsOfType = (array<array<GlobalItemRef> > function() : () { array<array<GlobalItemRef> > thing; thing.resize(eItemTypes.COUNT); return thing })()
	array<array<GlobalItemRef> > globalItemRefsOfType
...
{
	return file.PostInitFunctions
}
void function PreInitGlobal()
{
	file.globalItemRefsOfType.resize(eItemTypes.COUNT)
}
...
ItemData function CreateBaseItemData( int itemType, string ref, bool hidden )
{
	...
	item.ref = ref
	item.hidden = hidden
	//int LengthOfItemsOftype = file.itemsOfType.len()
...
array<ItemDisplayData> function GetVisibleItemsOfTypeWithoutEntitlements( entity player, int itemType, string parentRef = "" )
{
	...
	if ( player == null || GetModdedTitanClasses().contains(parentRef))
	{
		return items
	}
...
void function AddGenericItem( int dataTableIndex, int itemType, string ref, string name, string desc, string longdesc, asset image, int cost, bool isHidden )
{
	ItemData item 			= CreateBaseItemData( itemType, ref, isHidden )
	item.name               = name
	item.desc               = desc
...
bool function IsItemNew( entity player, string ref, string parentRef = "" )
{
	...
	if ( !IsSubItemType( refType ) )
	{
		...
		switch ( refType )
		{
			...
			case eItemTypes.TITAN:
				if(GetModdedTitanClasses().contains(ref)){
					return false
				}
...
void function ClearNewStatus( var button, string ref, string parentRef = "" )
{
	...
	ItemData itemData = GetItemData( ref )
	int refType = itemData.itemType
	if(GetModdedTitanClasses().contains(parentRef) || GetModdedTitanClasses().contains(ref))
		return
	if ( !IsSubItemType( refType ) )
	{
		int bitIndex = GetItemPersistenceId( ref )
...
{
	if(parentRef in GetModdedTitansByClassNoPersist()){
		return false
	}
	switch ( refType )
	{
