...
void function UpdateFDPanel( var panel, int index, bool compactMode = true )
{
	...
	if(loadout.titanClass in GetModdedTitansByClassNoPersist())
		titanClass = GetModdedTitanClassForMods(loadout.titanClass)
	int titanLevel = FD_TitanGetLevelForXP( titanClass, FD_TitanGetXP( player, titanClass ) )
	int currentXP = FD_TitanGetXP( player, titanClass )

	RuiSetInt( rui, "numFilledPips", FD_TitanGetLevelForXP( titanClass, currentXP ) )
	RuiSetInt( rui, "numPips", FD_TitanGetMaxLevel( titanClass ) )

	float frac = float( FD_TitanGetFilledPipsForXP( titanClass, currentXP ) ) / float( FD_TitanGetNumPipsForXP( titanClass, currentXP ) )

	var dataTable = GetDataTable( $"datatable/titan_properties.rpak" )
	int row = GetDataTableRowMatchingStringValue( dataTable, GetDataTableColumnByName( dataTable, "titanRef" ), titanClass )
	...
	{
		RuiSetString( rui, "titanLevelString", GetItemUnlockReqText( titanClass ) )
	}
	else
	{
